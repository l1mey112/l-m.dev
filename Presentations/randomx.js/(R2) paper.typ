...
- (2) Before a block is mined, the network sets a new difficulty. In Monero's case, they want some amount
      ...
      head against the wall pissing energy and heat into the wind.
...
- (3) Note that this is just some hash, it could be anything, it just needs to be some work.
*/
...
/*
- (2) We have that hashes equal votes. Your hashrate, the amount of hashes you can do
      in a second, or before the next block time, is your share in the control you
      have over the network.

      The more hashes, the higher chances it'll be your turn to mint a block and decide
      what transactions you want to process for the network.

- (3) If you control at least 51% of the possible hashrate out there, no one can stop you.
      It's like having 51% equity, or 51% stock. When you mine new blocks, a malicious
      actor can stuff whatever they want in there, valid or invalid transations, and
      even rewrite history because they control more than 50% of validation power.

- (4) This is what would be called a "51% attack."
      You want the people to be in control of the network, where no single entitiy has
      a majority stake in the hashrate of the network.
      
      This is the founding idea behind cryptocurrencies by the way, it's a shame that
      most of them suffer from what comes next.
*/
#focus-slide()[
...
- (2) For one of us, one of our commodity machines, we get vote.
      ...
      Even if everyone in this room brought all of their devices and chained them
      together and just started mining, we wouldn't beat a random person in Texas or
      China with cheap electricity and a couple ASICs.

      There is no winning that fight. One of them is a thousand of us.

- (3) Once ASICs become the only competitive way to mine, it's over for the for the
      average person trying to secure the network.
*/
#slide(
...
/*
- (2) That's the point right? That's why everyone has them. That's also why no one
      has an ASIC.
- (3) Essentially, this is specialisation. An ASIC is a perfect machine that
      executes only a specific task.

      Think of all the overhead a CPU needs to be general across all tasks?
*/
#slide(title: [(1.POW) What is the problem with hashing?])[
...
/*
- (2) We want to create a hash function so general, that the perfect machine to
      calculate those hashes looks exactly like a CPU. Then, there is no advantage
      to using an ASIC.

      ...

- (3) Then, the CPU becomes this perfect machine. We want to make any ASIC created
...
- (5) If you remember, the dataset is 2 GiBs of fixed memory, just reading, no
...
- (3) ...
      ...
      called light-verification, or light-mode.

      The most important thing to take from this slide is that the RandomX VM will
      ask for items, and those items are generated by executing 8 hash functions
...
- (1) So here I'm going to explain how we actually force ASICs to use external
      ...
      An example of something that is memory hard would be Argon2 if you know what
      that is, it's a very widespread function used to hash your passwords. The 256
      MiB cache that in the slide before is actually filled by Argon2 for a similar
      reason to what I'm about to describe.
...
- (4) ...
      ...
      An ASIC for fast mode mining at the time of RandomX's introduction
...
- (2) Remember that CPUs have caches for frequently acccessed memory, and
      ...
      with L1 being the closest to a core, L2 further, and L3 furthest.
...
- (6) Because we reasonably lose out on the ability to represent an infinite amount
...
- (4) If you do any of these primitive operations on floats, you can expect the
      ...
      round to nearest, it would be missing out on a lot a circuit space.
