...
- (1) ...
      ...
      An application specific integratic circuit. Incredibly expensive, incredibly
      specialised pieces of kit, which are designed to do one thing and do it as
      fast as possible.

      You want to dispell ASICs as much as possible, as they enable centralisation
      of hashrate, which is basically power here, in the few who can afford them.
      ASICs are outclassing honest everyday miners, by making it only competitive
      to mine with an ASIC. They're responsible for many 51% attacks on Bitcoin
      for example.
...
#slide(title: [(1.RX) RandomX])[
  ...
  *Defn.*  #strong()[_A_]pplication-#strong()[_S_]pecific-#strong()[_I_]ntegrated-#strong()[_C_]iruit --- ASIC are hyperoptimised circuits.
]
...
- (4) ... So CPUs are good at maximising ILP, instruction level parallelism. In the
      ...
      (so speculative execution). CPUs can also cache the instructions read, as well.
...
/*
- (1) Before I conclude this section, I want to go over floating point numbers and give am
...
- (5) Now, an important thing to understand is that if you have any experience
      ...
      only ever provide RN these other rounding modes are more low level, for
...
- (2) It could replace ads, but it's just used for stealing CPU time from hacked
...
- (1) ...
      ...
      "can this be implemented and not be terribly slow". Remember, JavaScript
      and WebAssembly only has RN.
...
- (1) ...
      ...
      Instead of generating JavaScript code and executing that, you can do something
...
- (2) ...
      ...
      I guess to rewrite the C++ source, is because it just made sense to me at the
      time, and the tooling was there for WASM, as C had intrinsics and so on.
      The original C++ was basically unreadable, so I hope my version can be of
      value to anyone willing to learn.
*/
...
- (3) And we have that...
      ...
      So, rounding functions either snap up, or snap down. Can we use this to our
      advantage?
*/
#slide(title: [(3.FP) How does rounding actually work?])[
...
- (8) But, lets imagine we had it! Just for summing two numbers, if we wanted to
      implement fadd_1, we  could just use RN* here to find the error, and snap the
      sum to always rounding down.

      Remember, if RN* existed, this is a valid algorithm we can write down, it
      doesn't just live in math land anymore.
*/
...
- (1) So, here is the benchmark I tried on a couple CPUs...
      ...
      float operations to only 2-3. However, the overhead is only 2x, so not bad.

      The softfloat though, that's 17x slower compared to baseline, so a little
      over than 9x slower compared to semifloat, so those branches hit hard.
      Since RandomX is going to do thousands per program iteration, these add up
      and I'm guessing that's where the estimation of < 1 H/s came from.
*/
...
- (1) Well, here is the multiplication function in its entirety. It is pretty big,
      ...
      about. So, the middle chunk is for calculating the actual residue, the rror.
      ...
      and c to avoid underflow. This is okay, because we only care about signed.

      If we didn't have infinity in our set of possible values, we could skip the
      third part, but since we do for E registers, we must include it. It's a rare
      branch, so it costs next to nothing.

      Finally, it's all fed to the same nextafter function.
*/
...
- (1) So, lets take a look at mul_residue. This is one of Dekker’s algorithms,
      ...
      This whole thing is still branchless, but I mean, just look
      at it, it's really really ugly. So, it's definitely going to be at
      least a little slower than addition.
      ...
      core of this entire function. We just want to extract the error term, which
      is a * b, minus the rounded a * b. Is there a way to skip all of this crap?
...
- (1) Now, this is the berkely softfloat version, and it's so small that it can 
      ...
      integer operations, because you need to align the floating points of each
      value.
...
- (4) Which, is much, much easier to do in software, compared to floating point
      ...
      to add them, there's no tricks evident in the algebra like with multiplication.
*/
...
#slide(title: [(3.FP) In short])[
  ...
  #uncoverb(3)[
    ...
    - This speeds up emulation of addition considerably, multiplication, not so much.
...
#new-section-slide[The End]

#slide(title: "The End")[

  Thank you everyone! It's Q&A time. #l-m-meta[(reference material below)]
  ...
  #box(inset: 1em)[
    ...
    *Error Free Transforms (EFTs)*
    ...
    - #l-m-meta[Arnold, J. (2012)] Workshop on Numerical Computing “Floating-Point Arithmetic.”
    
      https://indico.cern.ch/event/166141/sessions/125684/attachments/201414/282782/Arnold-FPWorkshop-Print.pdf

    - #l-m-meta[My question on SO (2024)] https://stackoverflow.com/questions/78776730

    #box(inset: 1em)[
      #l-m-meta[(disclaimer)] EFTs don't work on all FP values, most accept "normal" input #l-m-meta[(no NaN, no $plus.minus$inf, no subnormals)] with more conditions, we get lucky with RandomX's assumptions #l-m-meta[(which are chosen for VM perf reasons)].
      
        I wish I could go over all of these intricacies, but not enough time #l-m-meta[(had to cut a lot anyway)].
        
        #l-m-meta[(come talk to me after if you're interested)]
    ]
